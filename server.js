...
client.on("ready", () => {
    client.user.setActivity(" ❤️ Major.");
...
client.on("message", async message => {
    ...
    for (const içindeki of prefikslerim) {
        if (message.content.startsWith(içindeki)) major = içindeki;
    }
    if (!major) return;
    const args = message.content.slice(major.length).trim().split(/ +/g);
